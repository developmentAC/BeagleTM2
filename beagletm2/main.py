...
def main(client: str = "",
    ...
    if client.lower() == "browser":
        ...
        if data_file.is_file() == False:
            console.print(f"\t [bold red]Oh :poop:! Error with data-file loading. Exiting...")
            exit()  # :thumbs_down:

        csv_file = p.main(data_file, abs_only, make_db, save_less) # pass word list and run parser, pass abstract or full journal scanning option
...
def bigHelp():
    ...
    console.print("\t [bold blue] * Parser: App for searching for keywords in articles. Note: you must include a keyword file.")
    console.print("\t [bold blue] * Browser: App to visualize data after parsing job.")


    console.print(f"\n\t [bold blue] Execute, process abstracts only")
    console.print(f"\t :smiley: [bold cyan] poetry run beagletm2 --client parser --data-file kw_short.md --abs-only")


    console.print(f"\n\t [bold blue] Execute, process abstracts only and create db from results.")
    console.print(f"\t :smiley: [bold cyan] poetry run beagletm2 --client parser --data-file kw_short.md --make-db --abs-only")


    console.print(f"\n\t [bold blue] Execute, process whole articles.")
    console.print(f"\t :smiley: [bold cyan] poetry run beagletm2 --client parser --data-file kw_short.md --no-abs-only")


    console.print(f"\n\t [bold blue] Execute, process whole articles and create db from results of smaller size.")
    console.print(f"\t :smiley: [bold cyan] poetry run beagletm2 --client parser --data-file kw_short.md --make-db --no-abs-only --save-less")
